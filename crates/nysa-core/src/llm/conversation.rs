...
impl ConversationManager {
    ...
    pub async fn send_message(
        ...
        self.history
            .add_user_message(thread_id, user_message, &author_name, author_id)
            .await?;
        ...
        loop {
            ...
            if response.has_tool_calls() && self.tool_executor.is_some() {
                ...
                let tool_results = self
                    .process_tool_calls(thread_id, &response.tool_calls, &mut all_tool_executions)
                    .await?;

                // Add tool results to messages and continue loop
                for result in tool_results {
                    messages.push(create_tool_message(result.tool_call_id, &result.result));
                }
            } else {
        ...
        self.history
            .add_assistant_message(thread_id, final_response.content.as_deref(), None)
            .await?;
    ...
    pub async fn stream_message(
        ...
        Ok(stream.map(|delta_result| match delta_result {
    ...
    async fn process_tool_calls(
        ...
        for tool_call in tool_calls {
            ...
            self.history
                .add_tool_message(thread_id, tool_call_id, tool_name, &result)
                .await?;
