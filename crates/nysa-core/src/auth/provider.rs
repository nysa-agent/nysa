...
use crate::auth::{
    LinkingCodeError, LinkingCodeService, RateLimitResult, RateLimiter, Session, SessionError,
    SessionManager, compute_lookup_hash, generate_token, hash_token, verify_token,
};
use crate::database::entities::user::{
    ActiveModel as UserActiveModel, Column, Entity as UserEntity,
};
...
pub trait AuthProvider: Send + Sync {
    ...
    async fn generate_linking_code(
        &self,
        user_id: Uuid,
        ...
        service
            .generate_code(user_id, self.platform_name())
            .await
...
impl AuthService {
    ...
    pub async fn create_session(
        ...
        self.session_manager
            .create(user_id, platform, platform_session_id, metadata)
            .await
    ...
    pub async fn validate_platform_session(
        ...
        self.session_manager
            .validate_by_platform_session(platform, platform_session_id)
            .await
            .map_err(AuthError::from)
    }
    ...
    pub async fn redeem_linking_code(
        ...
        self.linking_service
            .redeem_code(code, platform, platform_id, metadata)
            .await
            .map_err(AuthError::InvalidLinkingCode)
    }
    ...
    pub async fn generate_linking_code(
        ...
        self.linking_service
            .generate_code(user_id, platform)
            .await
            .map_err(AuthError::InvalidLinkingCode)
    }
    ...
    pub async fn find_user_by_platform(
        ...
        for user in users {
            if let Some(profiles) = user.linked_profiles.as_object()
                && let Some(platform_data) = profiles.get(platform)
                && let Some(id) = platform_data.get("id")
    ...
    pub async fn get_user(
        &self,
        user_id: Uuid,
    ) -> Result<crate::database::entities::user::Model, AuthError> {
